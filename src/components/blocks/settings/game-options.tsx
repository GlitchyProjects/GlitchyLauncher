...
export function GameOptions() {
  ...
  const currentLocale = useLocale.getState().locale ?? "en";
  const language = localLanguage ?? (langQuery.data as string) ?? currentLocale;
  const exitOnLaunch =
    localExitOnLaunch ?? (exitOnLaunchQuery.data as boolean) ?? false;

  const handleLanguageChange = async (lang: string) => {
    setLocalLanguage(lang);
    useLocale.getState().setLocale(lang as "en" | "fa");
          ...
          <div className="flex items-center justify-between rounded-xl border border-border/40 bg-secondary/20 p-4">
            <div>
              ...
              <div className="text-[11px] text-muted-foreground">
                Swaps system core language and direction.
              </div>
            </div>
            <select
